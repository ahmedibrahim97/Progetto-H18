...
public class Servizioferrovario {
    ...
    public void stampaprezzi(int num,Prenotazione p){
        ...
        if(viaggip.isEmpty() && (num)<=viaggicorr.size()){
            d=viaggicorr.get(num-1).getPercorso().calcololunghezza(p.getStazionepartenza(),p.getStazionearrivo());
        ...
        if(!(viaggip.isEmpty()) && num<=viaggia.size()){
            ...
            d=i1.calcololunghezza(p.getStazionepartenza(),stazioneincroccio);
            d+=i2.calcololunghezza(stazioneincroccio, p.getStazionearrivo());
    ...
    public void visualizzaposti(int num){
        ...
        int i,j,k=0;
        if(viaggip.isEmpty() && !(viaggicorr.isEmpty())){
